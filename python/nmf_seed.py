...
if __name__ == '__main__':
    ...
    fftSettings = {'n_fft':seed_data['n_fft'],'hop_length':seed_data['hop_length']}
    
    audio_buffer, sr = librosa.load(args.audio_path_to_analyze,sr=None,mono=True)
    original_stft = librosa.stft(audio_buffer,**fftSettings)
    original_mags = np.transpose(np.abs(original_stft,dtype=np.float64))

    if args.bases_mode == 'random':
        bases = np.random.rand(seed_data['n_components'],original_mags.shape[1])
    elif args.bases_mode == 'seed':
        bases = np.array(seed_data['bases'])
    
    if args.activations_mode == 'random':
        activations = np.random.rand(original_mags.shape[0],seed_data['n_components'])
    ...
    new_acts = model.fit_transform(original_mags, W=activations, H=bases)
    ...
    plt.subplot(3, 1, 1)
    librosa.display.specshow(librosa.amplitude_to_db(np.abs(original_stft)), sr=sr, x_axis='time', y_axis='log')
